<?php
    ...
    class Operations extends ConnectionHandler{
        ...
        public function RegisterUser($Firstname,$Lastname,$Username,$Password,$email,$Country,$City,$Immobilier,$Juridique,$Financiere,$Informatique,$Intermediation,$gender) {
			$sql = "INSERT INTO users (firstName,lastName,userName,passwrd,email,country,city,immobilier,ingenieurie_Juridique,ingenieurie_Financiere,ingenieurie_Informatique,intermediation,gender) VALUES ('$Firstname','$Lastname','$Username','$Password','$email','$Country','$City','$Immobilier','$Juridique','$Financiere','$Informatique','$Intermediation','$gender')";
        ...
        public function SelectUser($var1, $var2){
                $sql = "SELECT * FROM users WHERE email=:email AND passwrd = :password";
                $this->statement = $this->connection->prepare($sql);
                $this->statement->bindParam(':email',$var1);
                $this->statement->bindParam(':password',$var2);
                $this->statement->execute();
                return $this->statement->fetch(PDO::FETCH_ASSOC);
        }
        public function SelectTerrain1($var2){
                $sql = "SELECT * FROM users WHERE userName=:loca";
                $this->statement = $this->connection->prepare($sql);
                $this->statement->bindParam(':loca',$var2);
                $this->statement->execute();
                return $this->statement->fetch(PDO::FETCH_ASSOC);   
        }
        public function SelectTerrain($var1,$var2){
                $sql = "SELECT * FROM $var1 WHERE location=:loca";
                $this->statement = $this->connection->prepare($sql);
                $this->statement->bindParam(':loca',$var2);
                $this->statement->execute();
                return $this->statement->fetch(PDO::FETCH_ASSOC);   
        ...
        public function addterrain($Location,$City,$Country,$filePath,$table,$descrip) {
            $sql = "INSERT INTO $table (location,city,country,file,description) VALUES ('$Location','$City','$Country','$filePath',',$descrip')";
            $this->statement = $this->connection->prepare($sql);
            if($this->statement->execute()){
                header("location: ../pages/Home.php");
            }
        ...
        public function getActivities($var){
            $sql = "SELECT * FROM users WHERE userName = :var1";
            $this->statement = $this->connection->prepare($sql);
            $this->statement->bindParam(':var1',$var);
            $this->statement->execute();
            return $this->statement->fetch(PDO::FETCH_ASSOC);
        }
        public function updateStatus($var4){
            $status = "Online";
            $sql = "UPDATE users set status = :statu WHERE userID = :uid";
            $this->statement = $this->connection->prepare($sql);
            $this->statement->bindParam(':statu',$status);
            $this->statement->bindParam(':uid',$var4);
            $this->statement->execute();
        }
        public function updateOffline($var){
            $status = "Offline";
            $sql = "UPDATE users set status = :statu WHERE user = :lname";
            $this->statement = $this->connection->prepare($sql);
            $this->statement->bindParam(':statu',$status);
            $this->statement->bindParam(':lname',$var);
            $this->statement->execute();
        }
